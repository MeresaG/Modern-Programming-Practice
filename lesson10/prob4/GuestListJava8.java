...
public class GuestListJava8 {
	...
	public void printListOfExpectedFemaleGuests(List<Guest> invited) {
		//implement
		String listOfGuests = invited.stream()
				.filter(g -> {
					try {
						return checkLegal(g);
					}catch (IllegalGuestException ige){
						System.out.println("Illegal Exception: " + ige.getMessage());
						throw new RuntimeException(ige);
					}
				})
		...
		System.out.println(listOfGuests.substring(1, listOfGuests.length()-1));
	}
